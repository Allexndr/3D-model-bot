...
# Use only transformers CLIP to avoid timm/open-clip memory issues
...
class ImageAnalyzer:
    def __init__(self):
        print("Загрузка CLIP-модели (lightweight версия)...")

        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        
        # Use only transformers CLIP to avoid memory issues
